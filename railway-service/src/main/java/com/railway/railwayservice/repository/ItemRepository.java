...
public interface ItemRepository extends JpaRepository<GoodsEntity,Long> {
    ...
    @Query(value = "SELECT g.id as id, g.name as name, g.description as description, g.date as date, g.minQuantity as minQuantity, g.totalQuantity as totalQuantity, g.unitsEntity as units, g.categoryEntity as category FROM GoodsEntity g WHERE g.categoryEntity = :itemCategory AND g.isDeleted = :isDeleted ORDER BY g.name ASC")
    ...
    @Query(value = "SELECT g.id as id, g.name as name, g.unitsEntity as unitsEntity FROM GoodsEntity g WHERE g.categoryEntity = :itemCategory AND g.isDeleted = :isDeleted ORDER BY g.name ASC")
